                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-4 gap-2">
                    {loading ? (
                        Array.from({ length: 9 }).map((_, idx) => (
                            <div
                                key={idx}
                                className="rounded-xl flex items-center gap-4 w-full h-24 border animate-pulse bg-gray-200"
                            >
                                <div className="w-20 h-full bg-gray-300 rounded-l-xl"></div>
                        ...
                        facultiesData?.data?.map((items, index) => (
                            ...
                            {/* <Link key={index} to={`/e-faculty/${items.id}`} className="group"> */}
                                ...
                                >
                                    <div className="flex justify-center items-center md:w-24 md:h-24 w-16 h-16 bg-white/95 md:p-3 p-2 shrink-0">
                                        <img 
                                            src={items.image} 
                                            alt={'Image 404'} 
