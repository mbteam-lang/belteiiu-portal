            ...
            <section className='max-w-7xl m-auto md:px-5 px-3'>
                {loading ? (
                    Array.from({ length: 10 }).map((_, index) => (
                        <div key={index} className="p-3 mt-3 border border-gray-200 dark:border-slate-700 shadow rounded-lg flex items-center gap-3 w-full bg-[#FFFFFF] dark:bg-[#353535] animate-pulse">
                        ...
                        {facultiesData && (
                            facultiesData.data?.map((items, index) => (
                                <Link key={index} to={`/major/${items.id}`} className="group">
                                    ...
                                    >
                                        <div className="flex justify-center items-center w-[80px] h-[80px] bg-white/95 dark:bg-[#353535]/95 p-3 shrink-0">
                                            <img
                                                src={items.image}
                                                alt={items.faculty}
                                                onError={(e) => {
