        ...
        <>
            {loading &&(
                Array.from({ length: 5 }).map((_, index) => (
                    <div
                        key={index}
                        className="flex items-center justify-between bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-lg shadow-sm px-3 py-1 animate-pulse max-w-7xl m-auto"
                    >
            ...
            <div className='bg-[#F5F5F5] dark:bg-slate-900 min-h-screen text-slate-800 dark:text-slate-100 transition-colors duration-200'>
                ...
                <section className='max-w-7xl m-auto md:px-5 px-2'>
                    {academicFaculty.length > 0 ? (
                        academicFaculty.map((items, index) => (
                            ...
                            <button
                                className="px-3 mt-3 border shadow rounded-lg flex items-center gap-3 w-full bg-[#FFFFFF] dark:bg-slate-800 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors"
