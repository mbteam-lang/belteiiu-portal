                ...
                {cslMenu.map((item, index) => (
                    <div className="space-y-4" key={index}>
                        <div className='flex items-center gap-4 border-b pb-4 mb-4'>
                            ...
                            <h2 className="md:text-xl text-lg font-semibold text-[#757575]">{t(item.title)}</h2>
                        </div>
                        {item.sub_programs.map((sub_program) => (
                            <div key={sub_program.id} className={`p-2 rounded-lg mt-3 ${sub_program.color} flex justify-between items-center cursor-pointer`} 
                                onClick={() => navigate(`/bis/csl-details/${sub_program.id}`)}>
                                <div className="flex-shrink-0 w-12 h-16 md:w-20 md:h-28 overflow-hidden rounded-lg mr-4">
                                    <img src={sub_program.image} alt="Certification Image" className="w-full h-full object-cover object-center " />
                                </div>
