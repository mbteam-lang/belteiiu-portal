...
import { formatDuration } from '../more/format.jsx';
...
export default function Index() {
    ...
    return (
        <>
            <div className={`${localStorage.getItem('darkMode') === 'dark' ? 'bg-[#181818] ' : 'bg-[c]'}`}>
                ...
                <section className="mt-10 md:w-11/12 md:px-0 px-5 mx-auto">
                    {loading ? (
                        <div className="skeleton-item md:w-60 w-32 mx-auto md:h-8 h-5 rounded-md bg-gray-200 animate-pulse mb-5" />
                    ) : (
                        <h1 className={`font-black md:text-3xl text-xl text-center mb-10 ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#4E4E4E]'}`}>{i18n.t('index.12fac')}</h1>
                    )}
                    {loading ? (
                        <div className="skeleton-loading">
                            <div className='grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 '>
                                {[...Array(12)].map((_, index) => (
                                    <div key={index} className="skeleton-item w-full h-16 rounded-md bg-gray-200 animate-pulse" />
                                ))}
                            </div>
                        </div>
                    ) : (
                            ...
                            {data.map((item, index) => (
                                <Link to={`/faculty/${item.id}`} className='overflow-hidden rounded-xl hover:scale-105 transition-transform duration-300' key={index} style={{ border: `1px solid ${item.color}` }}>
                                    <div className="h-20 w-full flex justify-start items-center gap-1 drop-shadow-md rounded-lg overflow-hidden  " style={{ backgroundColor: item.color }}>
                                        <div className="flex w-20 h-20 bg-white">
                    ...
                    <div className='flex gap-5 py-10'>
                        <div className='w-14 h-14 bg-[#D6EBED] rounded-full flex items-center justify-center'>
                        ...
                        <h1 className={` font-bold text-xl flex items-center ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#4E4E4E]'}`}>{i18n.t('index.pop_lesson')}</h1>
                    </div>
                            ...
                            <div className="skeleton-loading">
                                <div className='grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-3'>
                                    {[...Array(4)].map((_, indexPopCourse) => (
                                        <div key={indexPopCourse} className="skeleton-item w-full h-60 rounded-md bg-gray-200 animate-pulse" />
                                    ))}
                                </div>
                            </div>
                        ) : (
                                ...
                                {dataPopCourse.map((popCourse, indexPopCourse) => {
                                    ...
                                    return (
                                        <Link to={`/lessons/${popCourse.id}`} onClick={() => { handleClick(popCourse.id); }} key={indexPopCourse}>
                                            <div key={indexPopCourse} className={`rounded-xl shadow-md h-auto p-3  ${localStorage.getItem('darkMode') === 'dark' ? 'bg-[#303030] ' : 'bg-white'}`}>
                                                ...
                                                <div className="relative group text-left">
                                                    <div>
                                                        <h1 className={`text-md  line-clamp-2 p-1 mt-2 ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#4E4E4E]'}`}>{displayText}</h1>
                                                        {isTruncated && (
                                                ...
                                                <div className=' flex items-center gap-3 mt-3'>
                                                    ...
                                                    <div className='px-2 w-full'>
                                                        <h1 className={`text-md ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#8f8f8f]'}`}>{popCourse.faculty}</h1>
                                                        <div className='flex justify-between items-center'>
                                                            <h1 className={`text-sm ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#8f8f8f]'}`}>{i18n.t('index.view')}: {popCourse.view}</h1>
                                                            <div className='flex gap-1 items-center'>
                                                                <AccessTimeIcon className={`text-sm ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#8f8f8f]'}`} />
                                                                <h1 className={`text-sm mt-1 ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#8f8f8f]'}`}>{formatDuration(popCourse.duration)}</h1>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    <div className='flex gap-5 py-10'>
                        ...
                        <h1 className={`font-bold text-xl flex items-center ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#4E4E4E]'}`}>{i18n.t('index.new_lesson')}</h1>
                    </div>
                            ...
                            <div className="skeleton-loading">
                                <div className='grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-3'>
                                    {[...Array(4)].map((_, indexNewCourse) => (
                                        <div key={indexNewCourse} className="skeleton-item w-full h-60 rounded-md bg-gray-200 animate-pulse" />
                                    ))}
                                </div>
                            </div>
                        ) : (
                                ...
                                {dataNewCourse.map((newCourse, indexNewCourse) => {
                                    ...
                                    return (
                                        <Link to={`/lessons/${newCourse.id}`} onClick={() => { handleClick(newCourse.id); }} key={indexNewCourse}>
                                            <div key={indexNewCourse} className={`rounded-xl shadow-md h-auto p-3  ${localStorage.getItem('darkMode') === 'dark' ? 'bg-[#303030] ' : 'bg-white'}`}>
                                                ...
                                                <div className=' flex items-center gap-3 mt-3'>
                                                    ...
                                                    <div className='px-2 w-full'>
                                                        <h1 className={`text-md ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#8f8f8f]'}`}>{newCourse.faculty}</h1>
                                                        <div className='flex justify-between items-center'>
                                                            <h1 className={`text-sm ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#8f8f8f]'}`}>{i18n.t('index.view')}: {newCourse.view}</h1>
                                                            <div className='flex gap-1 items-center'>
                                                                <AccessTimeIcon className={`text-[10px] ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#8f8f8f]'}`} />
                                                                <h1 className={`text-sm mt-1 ${localStorage.getItem('darkMode') === 'dark' ? 'text-[#FFFFFF] opacity-75 ' : 'text-[#8f8f8f]'}`}>{formatDuration(newCourse.duration)}</h1>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
